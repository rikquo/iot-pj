...
import easyocr
# import serial  # <- uncomment if you want Arduino serial
...
def _init_log():
    if not os.path.exists(LOG_PATH):
        with open(LOG_PATH, "w", newline="", encoding="utf-8") as f:
            w = csv.writer(f)
            w.writerow(["timestamp","event","camera","plate",
...
def log_event(event, camera, plate, start_ts=None, end_ts=None,
    ...
    with open(LOG_PATH, "a", newline="", encoding="utf-8") as f:
        csv.writer(f).writerow([ts,event,camera,plate,start_str,end_str,
                                dur if duration_sec else "", dur_hms if duration_sec else "",
...
def best_plate_from_texts(texts: list[str]) -> str | None:
    ...
    if texts:
        ...
        if j: cand += [j, j.replace(" ", "")]
    for c in cand:
        p = normalize_plate(c)
        if p: return p
    return None

def detect_plate(yolo: YOLO, reader: easyocr.Reader, frame, seen_streak, last_ocr_time):
...
def main():
    ...
    # Optional Arduino (example):
    # ard = serial.Serial("COM6", 115200, timeout=1)  # adjust port
    # def send_open(which): ard.write((which+"\n").encode())
    ...
    try:
        while True:
            ...
            if ok_in:
                i += 1
                if i % STRIDE_ENTRY == 0:
                    ann_in, seen_in, plate_in, t_in = detect_plate(yolo, reader, frame_in, seen_in, t_in)
                    if plate_in:
                        ...
                        if plate_in not in active:
                            ...
                            log_event("START", "ENTRY", plate_in, start_ts=active[plate_in])
                            # send_open("ENTRY")  # signal Arduino
                    else:
                        ann_in = ann_in
                else:
                    ann_in = frame_in
                if last_plate_in:
                    ...
                    cv2.putText(ann_in, f"ENTRY: {last_plate_in}", (20,45),
            ...
            if ok_out:
                j += 1
                if j % STRIDE_EXIT == 0:
                    ann_out, seen_out, plate_out, t_out = detect_plate(yolo, reader, frame_out, seen_out, t_out)
                    if plate_out:
                        ...
                        if plate_out in active:
                            ...
                            log_event("RECEIPT", "EXIT", plate_out,
                                      start_ts=start, end_ts=now,
                                      duration_sec=duration, blocks=blocks, cost=cost)
                            # send_open("EXIT")  # signal Arduino to lift exit barrier
                    else:
                        ann_out = ann_out
                else:
                    ann_out = frame_out
                if last_plate_out:
                    ...
                    cv2.putText(ann_out, f"EXIT:  {last_plate_out}", (20,45),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0,255,255), 2, cv2.LINE_AA)
